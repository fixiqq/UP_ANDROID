            ...
            public void run() {
                try {
                    ...
                    if (HttpResult == HttpURLConnection.HTTP_OK) {
                        ...
                        Class_User.Id = response.getString("id");
                        Class_User.avatar = response.getString("avatar");
                        Class_User.nickName = response.getString("nickName");
                        Class_User.email = response.getString("email");
                        Class_User.token = response.getString("token");
                        Log.d("response",response.toString());

                        Intent main = new Intent(BoardingActivity.this,MainActivity.class);
                        startActivity(main);
                    } else {
                        Toast.makeText(BoardingActivity.this, "invalid username or password", Toast.LENGTH_SHORT).show();
                    }
                }
                catch (Exception e){
                    Log.d("e",e.toString());
